...
(function () {
  ...
  async function loadAutoGeneratedContent() {
    try {
      console.log('Loading auto-generated content from registry...');
      const response = await fetch('content-registry.json');
      if (!response.ok) {
        console.error('Failed to fetch content-registry.json:', response.status);
        return;
      }
      const registry = await response.json();
      console.log('Loaded registry with', registry.content.length, 'items');

      registry.content.forEach((item, index) => {
        const sectionId = registry.categories[item.category]?.section_id;
        const section = document.getElementById(sectionId);

        if (section) {
          // Find the story-grid div within the section
          let container = section.querySelector('.story-grid');
          // Fallback to section if no grid found
          if (!container) {
            container = section;
          }
          // Determine emoji and theme based on type
          const emojiMap = {
            'story': '📘',
            'maths': '🧮',
            'science': '🔬',
          ...
          const tagClass = tagColorMap[item.type] || 'tag-story';

          // Create story card HTML
          const storyCard = document.createElement('a');
          storyCard.className = 'story-card';
          storyCard.href = item.filename;

          // Extract age group for display
          const ageDisplay = item.age_group ? `Ages ${item.age_group}` : 'All Ages';

          storyCard.innerHTML = `
            <div class="card-cover" style="background: linear-gradient(135deg, #2a3f6a 0%, #3b5d9a 52%, #7ab6f9 100%);">
              ${emoji}
              <span class="badge">${item.category.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</span>
            </div>
            <div class="card-body">
              <div class="card-number">${item.type.charAt(0).toUpperCase() + item.type.slice(1)} #${index + 1}</div>
              <div class="card-title">${item.title}</div>
              <div class="card-desc">${item.description || ''}</div>
              <div class="card-tags">
                <span class="tag ${tagClass}">${item.type.charAt(0).toUpperCase() + item.type.slice(1)}</span>
                <span class="tag tag-age">${ageDisplay}</span>
                <span class="tag tag-science">Learning</span>
              </div>
            </div>
            <div class="card-footer">
              <span class="pages-info">${emoji} ${item.hook_line || 'Auto-generated content'}</span>
          ...
          console.warn('Section not found for category:', item.category, 'section_id:', sectionId);
        }
      });
    } catch (error) {
      console.error('Error loading auto-generated content:', error);
    }
  }

  document.addEventListener('DOMContentLoaded', loadAutoGeneratedContent);
